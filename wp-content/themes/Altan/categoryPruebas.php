...
query_posts('category_name=devices'); 
			...
			// 	echo the_field('f_cam'); 
			// 	echo the_field('b_cam');
			// 	echo the_field('memory');
			// 	echo the_field('cpu'); 
			// 	echo the_field('spectrum');
		 	echo '</ul>';
			 	endwhile; endif; 
  	wp_reset_query(); 
				...
				// 
				echo do_shortcode('[ajax_load_more post_type="post" category="devices" posts_per_page="4" scroll="false" transition="fade" transition_container="false" images_loaded="true" container_type="div" css_classes="posts" button_label="Mas"]');
				echo $current_cat->slug;
				echo $_GET["lang"];
			...
			print_r($categorias);
				...
				//$template = 'template_3';
				//echo do_shortcode('[ajax_load_more repeater="'.$template.'" container_type="div" css_classes="row" post_type="post" posts_per_page="6" category="dispositivos" scroll="false" transition="fade" transition_container="false" button_label="Ver más" button_loading_label="Cargando..."]');
				// $Marca y $Gama
				// if (isset($_GET['marca'])) {
				// 	$Marca = $_GET['marca'];
				// }else{$Marca = 'todas';}
				...
				// 	echo do_shortcode('[ajax_load_more repeater="'.$template.'" container_type="div" css_classes="row" post_type="post" posts_per_page="6" scroll="false" transition="fade" transition_container="false" button_label="Ver más" button_loading_label="Cargando..."]');
				// }
				// if ($Gama != 'todas' & $Marca == 'todas') {
				// 	$cualGama = $Gama;					
				// 	echo do_shortcode('[ajax_load_more repeater="'.$template.'" container_type="div" css_classes="row" post_type="post" posts_per_page="6" meta_key="gama" meta_value="'.$cualGama.'" meta_compare="IN" scroll="false" transition="fade" transition_container="false" button_label="Ver más" button_loading_label="Cargando..."]');
				// }
				// if ($Gama == 'todas' & $Marca != 'todas') {
				// 	$cualMarca = $Marca;
				// 	echo do_shortcode('[ajax_load_more repeater="'.$template.'" container_type="div" css_classes="row" post_type="post" posts_per_page="6" meta_key="marca" meta_value="'.$cualMarca.'" meta_compare="IN" scroll="false" transition="fade" transition_container="false" button_label="Ver más" button_loading_label="Cargando..."]');
				// }
				// if ($Marca != 'todas' & $Gama != 'todas') {
				// 	$cualGama = $Gama;
				// 	$cualMarca = $Marca;
				// 	echo do_shortcode('[ajax_load_more repeater="'.$template.'" container_type="div" css_classes="row" post_type="post" posts_per_page="6" meta_key="marca:gama" meta_value="'.$cualMarca.':'.$cualGama.'" meta_compare="IN:IN" meta_type="CHAR:CHAR" meta_relation="AND" scroll="false" transition="fade" transition_container="false" button_label="Ver más" button_loading_label="Cargando..."]');
				// }
				...
				// $Marca y $Gama
					...
					//echo do_shortcode('[ajax_load_more container_type="div" css_classes="row" repeater="template_3" post_type="post" posts_per_page="6" scroll="false" transition_container="false" images_loaded="true"]');
	...
	$(document).on('click', '.popmake-461', function(){
      ...
      // });
		...
		$('a.filtrarEN').click(function(){
			...
			window.location = '<?php echo get_site_url(); ?>/category/devices/?lang=en/?gama='+gama+'&marca='+marca;
		});
