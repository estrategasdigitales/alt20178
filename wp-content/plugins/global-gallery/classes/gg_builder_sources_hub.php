...
class gg_builder_hub {
	...
	public function is_gallery_ready($check_db = false) {
		...
		if(in_array($this->src, array('fb', 'picasa', 'g_drive')) && !$this->gall_params['connect_id']) {
			return false;
		}
		if(!in_array($this->src, array('wp', 'wp_cat', 'cpt_tax', 'gg_album', 'ngg', 'picasa', 'g_drive', 'fb')) && !$this->gall_params['username']) {
			return false;
		}
