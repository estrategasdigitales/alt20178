(function($) {
	...
	gg_galleries_init = function(gid, after_resize) {
		...
		if(typeof(gid) != 'undefined' && gid) {
			if(!jQuery('#'+gid).length) {return false;}
		...
		if(gg_new_images[gid]) {
			jQuery('#'+gid+' .gg_img .gg_main_thumb').lcweb_lazyload({
				allLoaded: function(url_arr, width_arr, height_arr) {
					...
					jQuery(window).trigger('gg_loaded_gallery', [gid]);
				}
			});
		}
		else {
			...
			jQuery('#'+cid+' .gg_coll_img .gg_main_thumb').lcweb_lazyload({
				allLoaded: function(url_arr, width_arr, height_arr) {
					...
					setTimeout(function() { // litle delay to allow masonry placement
						...
						gg_new_images[cid] = 0;
						jQuery(window).trigger('gg_loaded_collection', [cid]);
					}, 300);
				}
			});
		}
	...
	gg_load_coll_gallery = function(cid, gdata) {
		...
		setTimeout(function() {
			jQuery('#'+cid+' .gg_coll_table_first_cell').css('position', 'absolute');
		}, 710);
	...
	jQuery(document).ready(function() {
		jQuery('body').delegate('.gg_coll_go_back', 'click', function() {
			...
			// go back
			jQuery('#'+cid+' .gg_coll_table_first_cell').css('opacity', 1).css('position', 'static');		
		...
		jQuery('#gg_car_'+gid+' '+ shown_first +' img').lcweb_lazyload({
			allLoaded: function(url_arr, width_arr, height_arr) {
				...
				setTimeout(function() {
					...
					jQuery(window).trigger('gg_loaded_carousel', [gid]);
				}, delay);
			}
		});
	}
	...
	if(typeof(gg_preload_hires_img) != 'undefined' && gg_preload_hires_img) {
		var $phi_subjs = jQuery('.gg_gallery_wrap, .gg_carousel_wrap');
		var phi_tot_subjs = $phi_subjs.length;
		var phi_loaded = 0;
		
		if(phi_tot_subjs) {
			jQuery(window).on('gg_loaded_gallery gg_loaded_collection gg_loaded_carousel', function() {
				phi_loaded++;
				
				if(phi_loaded == phi_tot_subjs) {
					setTimeout(function() {
						jQuery('.gg_img').not('.gg_coll_img, .gg_linked_img').each(function() {
							jQuery('<img />')[0].src = jQuery(this).data('gg-url');
						});	
					}, 300);
				}
			}); 
	...
	jQuery(document).ready(function() {
		jQuery(document).delegate('.gg_gallery_wrap div.gg_img:not(.gg_coll_img, .gg_linked_img)', 'click', function(e) {	
			e.preventDefault();	
			if( jQuery(e.target).hasClass('.ggom_socials') || jQuery(e.target).parents('.ggom_socials').length) {return false;}
